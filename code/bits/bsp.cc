...
namespace rCMI
{
  BSPTree::BSPTree(gf::RectI initialSpace)
  { // constructeur du BSPTree
    left = nullptr;
    right = nullptr;
    space = initialSpace;
  }
  ...
  void BSPTree::recursiveSplit(gf::Random &random)
  { // on va appeler récursivement la méthode split
    assert(!left && !right);
    ...
    { // si notre rectangle que l'on visite est trop grand alors on le split
      if (split(random))
      { // appel du split
        assert(left);
  ...
  Dungeon BSP::generate(gf::Random &random, gf::Vector2i sizeMap)
  {
    switch (getPhase())
    {
    case Phase::Start:
  ...
  void BSP::generateRooms(gf::Vector2i sizeMap)
  {
    m_dungeon = Dungeon(sizeMap, TileType::Wall);

    m_root.space = gf::RectI::fromPositionSize({0, 0}, sizeMap);
    m_root.left = nullptr;
